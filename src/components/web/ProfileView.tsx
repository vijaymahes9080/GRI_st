          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950/60 p-6 rounded-3xl border border-slate-800 space-y-6 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-3 pt-2">
              <button
                ...
                className="w-full py-3 px-4 rounded-xl bg-white hover:bg-slate-100 text-slate-900 font-bold text-xs flex items-center justify-center gap-3 transition shadow-lg shadow-black/30 disabled:opacity-50"
              >
                ...
                <span>{isGoogleLoading ? 'Connecting to Google...' : 'Sign In with Google (Firebase Auth)'}</span>
              </button>

              {authError && (
                <p className="text-xs text-rose-400 text-center bg-rose-950/40 border border-rose-800 p-2 rounded-lg">
                  {authError}
                </p>
              )}
            </div>
